...
bool strictOrientation(Point a, Point b, Point c) {
    Triangle t = {a, b, c};
    return (t.signedArea() > 0);
}

bool looseOrientation(Point a, Point b, Point c) {
    Triangle t = {a, b, c};
    return (t.signedArea() >= 0);
}
...
struct Polygon {
    ...
    vector<Point> convexHull() {
        ...
        upper.push_back(v[0]), lower.push_back(v[0]);
        int n = (int) v.size();
        for (int i = 1; i < n; i++) {
            while (upper.size() >= 2 && looseOrientation(upper[upper.size() - 1], upper[upper.size() - 2], v[i])) {
                upper.pop_back();
            }
            upper.push_back(v[i]);
            while (lower.size() >= 2 && !strictOrientation(lower[lower.size() - 1], lower[lower.size() - 2], v[i])) {
                lower.pop_back();
            }
            lower.push_back(v[i]);
        }
...
void solve() {
    ...
    vector<Point> v = polygon.convexHull();
    cout << v.size() << endl;
    for (Point i: v) {
        i.print();
        cout << " ";
