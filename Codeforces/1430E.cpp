#include <iostream>
#include <vector>
#include <map>
#include <set>
#include <unordered_set>
#include <algorithm>
#include <random>
#include <chrono>
#include <queue>
 
using namespace std;
int cl(int x){
    int pwr = 1;
    while(pwr < x){
        pwr *= 2;
    }
    return pwr;
}
pair<int,int> merge(pair<int,int> p1, pair<int,int> p2){
    return make_pair(p1.first,p2.second);
}
struct segmentTree {
    vector<long long> arr;
    vector<long long> v;
    vector<pair<long long,long long>> vec;
    long long build(int ind) {
        if (ind >= v.size() - 1) {
            arr[ind] = v[ind - (v.size() - 1)];
            return arr[ind];
        }
        arr[ind] = build(2 * ind + 1) + build(2 * ind + 2);
        return arr[ind];
    }
    pair<int, int> build2(int ind) {
        if (ind >= v.size() - 1) {
            int x = ind - (v.size() - 1);
            vec[ind] = make_pair(x, x);
            return vec[ind];
        }
        vec[ind] = merge(build2(2 * ind + 1), build2(2 * ind + 2));
        return vec[ind];
    }
    long long interval(int i, int L, int R) {
        if (vec[i].first > R || vec[i].second < L) {
            return 0;
        }
        if (vec[i].first >= L && vec[i].second <= R) {
            return arr[i];
        }
        long long a = interval(2 * i + 1, L, R);
        long long b = interval(2 * i + 2, L, R);
        return a + b;
    }
    void update(int k, int u) {
        int ind = (v.size() - 1) + k;
        arr[ind] = u;
        while (ind != 0) {
            int parent = (ind - 1) / 2;
            arr[parent] = (arr[2 * parent + 1] + arr[2 * parent + 2]);
            ind = parent;
        }
    }
};
long long solve(vector<long long> ans){
    segmentTree segTree;
    int n = ans.size();
    int x = cl(n);
    vector<long long> v;
    vector<long long> arr;
    vector<pair<long long,long long>> vec;
    v.resize(x);
    long long counter = 0;
    for(int i = 0; i < n; i++){
        v[i] = 0;
    }
    for(int i = n + 1; i < x; i++){
        v[i] = 0;
    }
    arr.resize(2 * x);
    vec.resize(2 * x);
    segTree.arr = arr;
    segTree.v = v;
    segTree.vec = vec;
    segTree.build(0);
    segTree.build2(0);
    for(int i = 0; i < n; i++){
        int a = ans[i];
        segTree.update(a,1);
        int sum = segTree.interval(0, a + 1, n - 1);
        counter += sum;
        //cout << sum << endl;
    }
    return counter;
    //cout << counter << endl;
}
string des, cur;
int main() {
    int n;
    cin >> n;
    cin >> cur;
    des = cur;
    reverse(des.begin(), des.end());
    map<char,vector<long long>> myMap[2];
    for(int i = 0; i < cur.size(); i++){
        myMap[0][cur[i]].push_back(i);
        myMap[1][des[i]].push_back(i);
    }
    vector<long long> ans(n);
    for(char c = 'a'; c <= 'z'; c++){
        vector<long long> v1 = myMap[0][c];
        vector<long long> v2 = myMap[1][c];
        for(int i = 0; i < v1.size(); i++){
            ans[v1[i]] = v2[i];
        }
    }
    cout << solve(ans) << endl;
}
