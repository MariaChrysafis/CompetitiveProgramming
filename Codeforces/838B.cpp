...
int main() {
    ...
    while(Q--) {
        ...
        if (tc == 1) {
            ...
            cin >> v;
            u--, v--;
            int ans = st.get(t.first[u], t.last[u]) -st1.get(t.first[u], t.first[u]) + st1.get(t.first[v], t.first[v]);
            if (t.isAncestor(u, v)) {
                ans = min(ans, abs(st1.get(t.first[u], t.first[u]) - st1.get(t.first[v], t.first[v])));
            }
            cout << ans << endl;
        }
    }
