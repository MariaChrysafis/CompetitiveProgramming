...
int overCount(vector<coord> v) {
  ...
  int counter = 0;
  for (int i = 0; i < n; i++) {
    coord currentMin = v[i];
    coord currentMax = v[i];
    for (int j = i + 1; j < n; j++) {
       currentMin = currentMin.min(v[j]);
       currentMax = currentMin.max(v[j]);
      for (int k = j + 1; k < n; k++) {
        currentMin = currentMin.min(v[k]);
        currentMax = currentMin.max(v[k]);
        for (int l = k + 1; l < n; l++) {
          currentMin = currentMin.min(v[l]);
          currentMax = currentMin.max(v[l]);
