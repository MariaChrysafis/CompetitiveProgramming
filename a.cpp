...
/*
 * We can have at most 15 connected components
 */
...
void update_distances (int src) {
    ...
    for (int i = 0; i < grid.size(); i++) {
        di[i].assign(grid.size(), INF);
    }
    deque<pair<pair<int,int>,int>> q; //0-1 BFS (since each edge is either 0 or 1)
...
int main() {
    ...
    for (int i = 0; i < cc.size(); i++) {
        ...
        for (auto& p: cc[i]) {
            ...
            assert(comp[p.first][p.second] == i);
        }
    }
    dist.resize(cc.size());
    for (int i = 0; i < cc.size(); i++) {
